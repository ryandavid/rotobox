...
#else
                callback(offset+index, demod_buf_a, rs_0, FRAME_TYPE_ADSB);
#endif
                bit = startbit + skip_0;
                continue;
...
#else
                callback(offset+index+1, demod_buf_b, rs_1, FRAME_TYPE_ADSB);
#endif
                bit = startbit + skip_1;
                continue;
            } else {
...
#else
                callback(offset+index, demod_buf_a, rs_0, FRAME_TYPE_UAT);
#endif
                bit = startbit + skip_0;
                continue;
            } else if (skip_1 && rs_1 <= rs_0) {
...
#else
                callback(offset+index+1, demod_buf_b, rs_1, FRAME_TYPE_UAT);
#endif
                bit = startbit + skip_1;
                continue;
            } else {
